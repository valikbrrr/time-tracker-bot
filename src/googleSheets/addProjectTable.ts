...
export const addDataToProjectSheet = async (
  name: string,
  id: string,
  hours: string[],
  ...
  try {
    ...
    const foundProject = await timeTrackerProjModel.findOne({
    ...
    if (foundProject && foundProject.data.length > 0) {
      ...
      if (rowIndexToDelete !== null && rowIndexToDelete >= 0) {
        ...
        if (existingHours) {
          totalHours += existingHours
            .split(", ")
            .reduce(
              (sum: number, hour: string) => sum + (parseFloat(hour) || 0),
              0
            );
        }
      ...
      const newHoursTotal = hours.reduce(
