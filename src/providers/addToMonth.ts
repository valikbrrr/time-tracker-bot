...
export const addToMonth = async (
  ...
  const mainArr = await timeTrackerMonthModel.findOne({
    monthAndYear: `${selectedMonth} ${currentYear()}`,
  });
  ...
  return await timeTrackerMonthModel.updateOne(
    { monthAndYear: `${selectedMonth} ${currentYear()}` },
    { data: updateArr }
  );
};
