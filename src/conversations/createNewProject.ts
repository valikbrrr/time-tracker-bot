...
export async function createNewProject(
  ...
  const inlineKeyboard = new InlineKeyboard()
    .text("Сохранить название и продолжить", "nextStepCreateProject")
    .row()
  ...
  await ctx.reply(
    `Поздравляю! Вы создали новый проект под названием: "${projectName}" 🥳`,
    {
  ...
  const callbackQuery = await conversation.waitForCallbackQuery([
    "nextStepCreateProject",
    "BackToCreateProject",
  ]);
  if (callbackQuery.callbackQuery.data === "nextStepCreateProject") {
    const inlineKeyboardTwo = new InlineKeyboard().text(
      "Открыть список проектов",
      "callbackOpenProjectList"
    );
    ...
    await ctx.api.editMessageText(
      chatId,
      messageId,
      " Вы создали новый проект, теперь вы можете открыть список проектов и добавить часы в свой проект",
      {
    ...
    try {
      ...
      await ctx.reply(
        "Не удалось обновить сообщение. Пожалуйста, попробуйте еще раз."
      );
    }
